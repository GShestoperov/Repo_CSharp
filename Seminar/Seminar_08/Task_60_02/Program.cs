...
int[] CreateFrequencyArray(int[,] srcArray)
{
    if (srcArray.GetLength(0) == 0 || srcArray.GetLength(1) == 0)
        return new int[0];

    int max = srcArray[0, 0];
    ...
    for (int i = 0; i < srcArray.GetLength(0); i++)
        for (int j = 0; j < srcArray.GetLength(1); j++)
        {
            resultArray[srcArray[i, j]]++;
        }

    return resultArray;
}

void OutputFrequencyArray(int[] freqArray)
{
    for (int i = 0; i < freqArray.Length; i++)
    {
        if (freqArray[i] > 0)
            Console.WriteLine($"{i} встречается {freqArray[i]} раз");
    }
}
...
OutputArray(array);
int[] frequencyArray = CreateFrequencyArray(array);
