...
int[] CreateFrequencyArray(string srcStr)
{
    ...
    {
        resultArray[srcStr[i]]++;
    }

    return resultArray;
}

void OutputFrequencyArray(int[] freqArray, int srcLength)
{
    for (int i = 0; i < freqArray.Length; i++)
    {
        if (freqArray[i] > 0)
            Console.WriteLine($"Символ {(char)i} встречается {freqArray[i]} раз. " +
                $"Частота {Math.Round(((double)freqArray[i]) / ((double)srcLength) * 100, 2)}%");
    }
}
